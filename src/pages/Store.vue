  ...
  <div class="text-primary p-5">
    ...
    <div v-if="store.getters.isReady" class="my-5 text-primary text-xl grid gap-5">
      ...
      <div class="grid gap-3">
        <template v-for="item in neededItems">
          <div
            class="store-list-item"
            :class="item.added ? 'down' : 'up'"
            @click="toggleItem(item.id)"
          >
            <div>{{ item.name }}</div>
            <div>{{ item.count }} / {{ item.max }}</div>
          </div>
        </template>
        <template v-for="item in unneededItems">
          <div class="store-list-item down">
            <div>{{ item.name }}</div>
            <div>{{ item.count }} / {{ item.max }}</div>
          </div>
        </template>
...
const checkout = () => {
	const items = neededItems.value.filter(e => e.added)
